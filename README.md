...
#### 구조적 오류의 근본 원인: 표의 논리적 제약과 회귀 최적화 간의 불일치 (Misalignment)
DETR 계열의 탐지 모델은 각 객체(셀, 행, 열)를 독립적으로 탐지하고 정답 좌표와의 픽셀 단위 거리를 줄이는 **회귀(Regression) 최적화** 방식을 따릅니다. 반면, 표 구조 인식(TSR)은 행과 열 경계가 완벽히 맞아떨어져야 하는 이산적인 **논리 구조(Logical Structure)**를 복원하는 문제입니다. 탐지 모델은 위상학적 정렬 제약(Topological Alignment)을 알지 못하기 때문에, Spanning cell 추론에서 발생하는 단 1~2픽셀의 기하학적 오차가 치명적인 위상 왜곡(인접 행·열 침범 및 병합)으로 증폭됩니다. 즉, 실패의 본질은 '정답 좌표의 부재'가 아니라 **'독립적인 픽셀 오차 최적화'와 '위상학적/이산적 구조 제약' 간의 근본적인 불일치**입니다.
